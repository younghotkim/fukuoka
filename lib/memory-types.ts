...
export type Memory = {
  ...
  photoUrl: string;
  photos: string[];
  photoCaptions: Record<string, string>; // url → caption (one-line description per photo)
...
export function normalizeMemory(value: unknown): Memory {
  ...
  if (candidate.photoCaptions && typeof candidate.photoCaptions === "object" && !Array.isArray(candidate.photoCaptions)) {
    for (const [url, caption] of Object.entries(candidate.photoCaptions as Record<string, unknown>)) {
      if (typeof url === "string" && typeof caption === "string" && caption.trim()) {
        photoCaptions[url] = caption;
      }
    }
  }
  const expensePayer =
